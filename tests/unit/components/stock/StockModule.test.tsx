...
/**
 * ประเภทฟิล์ม / ความหนา / รหัสสี are one spec and they belong to the product —
 * "TPU-PR" is one film, one thickness, one colour. The shop was retyping the
 * last two on every service visit; storing them here is what lets a ticket and
 * its ใบเซอร์วิส read the same values without asking again.
 */
describe('StockModule — ความหนา / รหัสสี ของฟิล์ม', () => {
  it('saves the film spec with the product', async () => {
    const user = userEvent.setup();
    const saveProduct = vi.fn(async () => {});
    render(
      <StockModule
        stock={[{ ...stock[0], filmThickness: '195', filmColourCode: 'BK-01' }]}
        canDo={() => true}
        canSeeStockPrices={false}
        actions={{ saveProduct }}
      />,
    );
    await user.click(screen.getByLabelText('แก้ไขสินค้า SKU-FLM-3M60'));

    // Round-trips: what was stored comes back into the form.
    expect(screen.getByLabelText('ความหนาฟิล์ม')).toHaveValue('195');
    await user.clear(screen.getByLabelText('รหัสสีฟิล์ม'));
    await user.type(screen.getByLabelText('รหัสสีฟิล์ม'), 'BK-02');
    await user.click(screen.getByRole('button', { name: 'บันทึก' }));

    expect(saveProduct).toHaveBeenCalledWith(
      expect.objectContaining({ filmThickness: '195', filmColourCode: 'BK-02' }),
    );
  });

  it('does not ask for a film spec on a product that is not film', async () => {
    const user = userEvent.setup();
    render(
      <StockModule
        stock={[
          { ...stock[0], id: 9, sku: 'AMP-009', name: 'เพาเวอร์แอมป์', category: 'เครื่องเสียง' },
        ]}
        canDo={() => true}
        canSeeStockPrices={false}
      />,
    );
    await user.click(screen.getByLabelText('แก้ไขสินค้า AMP-009'));
