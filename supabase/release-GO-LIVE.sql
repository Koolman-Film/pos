...
--
-- รวม release-0019 ถึง release-0051 และ repair-categories-and-services.sql
...
where status = 'รออนุมัติ' and approved_at is null
  and not exists (select 1 from supabase_migrations.schema_migrations where version = '0050');
...
insert into supabase_migrations.schema_migrations(version, name) values ('0050', 'adjustment_approval') on conflict (version) do nothing;
...
insert into supabase_migrations.schema_migrations(version, name) values ('0051', 'price_decision_trail') on conflict (version) do nothing;
