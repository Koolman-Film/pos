...
--
-- รวม release-0019 ถึง release-0070 และ repair-categories-and-services.sql
...
-- supabase/release-0066.sql
--
-- ใบงานที่ปิดแล้ว: ช่างกรอกจำนวนสินค้าที่ใช้จริงย้อนหลังได้
--
-- รันต่อจาก release-0065.sql
--
-- ปลอดภัยเมื่อรันซ้ำ: มีแต่ create or replace function กับ trigger ที่ drop ก่อนสร้าง
--
-- หลังรันไฟล์นี้ ใบงานที่ส่งมอบและชำระครบแล้วจะยังกรอก "ข้อมูลของช่าง" ได้
-- ตราบใดที่จำนวนสินค้าที่ใช้จริงยังไม่ครบ หรือติ๊ก "แก้งาน" ไว้ ส่วนราคา
-- และการรับเงินยังล็อกเหมือนเดิม — และแน่นกว่าเดิม เพราะราคาในรายการสินค้า
-- ของใบงานที่ล็อกแล้ว ยิงตรงเข้าฐานข้อมูลก็แก้ไม่ได้อีกต่อไป
...
insert into supabase_migrations.schema_migrations(version, name) values ('0066', 'tech_after_lock') on conflict (version) do nothing;


-- ==========================================================================
-- supabase/release-0067.sql
-- ==========================================================================

-- supabase/release-0067.sql
--
-- เคลมประกันได้โดยไม่ต้องเป็นรอบเซอร์วิส และไม่กินสิทธิ์ของลูกค้า
--
-- รันต่อจาก release-0066.sql
--
-- ปลอดภัยเมื่อรันซ้ำ: เพิ่มคอลัมน์แบบ if not exists, เช็ค constraint ก่อนสร้าง
-- และ create or replace function
--
-- หลังรันไฟล์นี้ การเข้ารับบริการมีสองชนิด: "เซอร์วิส" (นับในแพ็กเกจที่ลูกค้าซื้อ)
-- กับ "เคลมประกัน" (มาเคลมอย่างเดียว ไม่กินสิทธิ์ นับเลขครั้งที่แยกของตัวเอง)
-- ของเดิมทุกแถวเป็น "เซอร์วิส" เหมือนเดิม ไม่มีอะไรขยับ
...
insert into supabase_migrations.schema_migrations(version, name) values ('0067', 'claim_without_service') on conflict (version) do nothing;
