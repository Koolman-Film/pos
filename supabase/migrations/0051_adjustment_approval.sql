-- supabase/migrations/0051_adjustment_approval.sql
--
...
alter table order_adjustments
  ...
  add column reject_note text not null default '';

alter table order_adjustments
  add constraint order_adjustments_status_ck
  check (status in ('รออนุมัติ', 'อนุมัติแล้ว', 'ปฏิเสธ'));
...
create index order_adjustments_status_idx on order_adjustments (status);
