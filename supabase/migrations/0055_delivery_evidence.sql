-- supabase/migrations/0055_delivery_evidence.sql
--
-- เปลี่ยนเป็น "จัดส่งแล้ว" ต้องกรอกข้อมูลการจัดส่ง และแนบไฟล์
--
-- ขายส่งส่งของก่อน แล้วค่อยเก็บเงินทีหลัง ซึ่งแปลว่าช่วงเวลาที่เสี่ยงที่สุดของธุรกิจ
-- คือหลังของออกจากคลังไปแล้วแต่เงินยังไม่เข้า. When a customer says the goods
-- never arrived, the only thing that settles it is what was written down at the
-- time: who carried it, which tracking number, and a photograph of the signed
-- delivery note. Until now the module recorded a DATE and nothing else.
--
-- So the transition to จัดส่งแล้ว — the one that also takes the stock off the
-- shelf (0054) and starts the revenue clock (0045) — now carries its evidence
-- with it, and the database refuses the transition without it.
--
-- **บังคับที่ฐานข้อมูล ไม่ใช่แค่ที่ฟอร์ม.** A required field in a form is a
-- suggestion: the Server Action behind it is a plain POST, and the trigger from
-- 0052 already governs this transition. Adding the rule anywhere else would
-- leave the one path that matters — somebody in a hurry, or a script — able to
-- mark a PO delivered with nothing behind it.
--
-- ของเก่าไม่ถูกแตะ. The rule fires only when the status CHANGES to จัดส่งแล้ว, so
-- every PO already sitting in that status stays exactly as it is. Nothing is
-- backfilled and nothing is demanded retroactively for deliveries that have
-- already happened.
...
alter table orders
  ...
  add column delivery_attachments text[] not null default '{}';
