-- supabase/migrations/0047_sales_people.sql
--
...
create index sales_people_shop_idx on sales_people (shop_id, sort_order) where active;
...
alter table orders
  ...
  add column sales_by text not null default '';

comment on column orders.sales_by is
  'พนักงานขายที่ขาย PO ใบนี้ — ชื่อ ไม่ใช่ FK. ว่างหมายถึงยังไม่ได้ระบุ.';

create index orders_sales_by_idx on orders (shop_id, sales_by) where sales_by <> '';
