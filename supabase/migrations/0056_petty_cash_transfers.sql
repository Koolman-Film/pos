-- supabase/migrations/0056_petty_cash_transfers.sql
--
-- เติมเงินสดย่อยจากหน้าค่าใช้จ่าย ต้องเข้ายอดเงินด้วย
--
-- ตั้งแต่ 0043 ยอดคงเหลือของทุกแหล่งเงินคิดจาก `money_transfers` — การเติมเงินสดย่อย
-- คือเงินที่ย้ายจากที่หนึ่งไปอีกที่หนึ่ง ไม่ใช่รายรับหรือรายจ่าย. 0043 copied every
-- top-up that existed at the time across ONCE. But the เติมเงินสดย่อย button on
-- บัญชี/ค่าใช้จ่าย kept writing to `petty_cash` alone, so every top-up pressed
-- there since go-live has been missing from the เงินสดย่อย balance — and the
-- account the cash actually came out of never went down either.
--
-- Nothing errored. Two screens simply disagreed about how much petty cash there
-- was, and the one the shop reconciles against was the wrong one.
--
-- สามอย่างในไฟล์นี้
--
--   1. `topup_petty_cash` — the button now writes BOTH rows in one transaction:
--      the petty-cash entry the accounting screen lists, and the transfer the
--      balance is built from. It also records where the money came from, which
--      the old rows never did. Two inserts from a Server Action could half-land;
--      one function cannot.
--
--   2. `petty_cash.money_transfer_id` ties the two together, so "has this top-up
--      reached the balance?" is a lookup rather than a guess. Existing rows are
--      linked below where a transfer unmistakably matches.
--
--   3. The rest are listed on /money for somebody to decide, one at a time —
--      `link_petty_cash_topup` imports one as a transfer, `skip_petty_cash_topup`
--      records that it was already keyed there by hand.
--
-- **ไม่คัดลอกของเก่าให้อัตโนมัติ.** Some of these top-ups were probably also
-- keyed as transfers on /money, on a different day or with a different note. A
-- blanket copy would count that money twice and there would be no way to tell
-- afterwards which of the two rows was the duplicate. Only an exact match — same
-- branch, same day, same amount, into the petty account, not already claimed — is
-- linked here; everything else waits for the person who knows.
