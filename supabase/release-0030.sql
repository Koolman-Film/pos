-- supabase/release-0030.sql
--
-- ดัชนีของตารางลูก — แก้หน้าเว็บช้า
--
-- รันก่อนได้เลย ไม่ต้องรอไฟล์อื่น — ข้ามตารางที่ยังไม่มีให้เอง
--
-- ปลอดภัยเมื่อรันซ้ำ: create index if not exists อย่างเดียว ไม่แก้ข้อมูล
--
...
insert into supabase_migrations.schema_migrations(version, name) values ('0030', 'hot_path_indexes') on conflict (version) do nothing;
