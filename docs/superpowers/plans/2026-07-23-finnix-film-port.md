...
**Tech Stack:** Next.js 16 (App Router, TypeScript, Turbopack default) + React 19, Supabase (`@supabase/supabase-js`, `@supabase/ssr`), Tailwind CSS v4 (CSS-first), `react-chartjs-2` + `chart.js`, `xlsx` (SheetJS), Vitest + React Testing Library v16+ (unit/component tests), Playwright (e2e), Supabase CLI (local dev stack + migrations). Exact versions self-select from `create-next-app@latest` (Task 1) — do not hardcode version numbers in `package.json`; match the idioms of whatever it installs.
...
- **Framework baseline (verified 2026-07-23 against official docs / npm — see `docs/framework-verification-2026-07-23.md`).** The stack is Next.js 16 + React 19 + Tailwind v4. Every task MUST honor these consequences, which differ from pre-2026 Next/Tailwind knowledge:
  1. `cookies()`, `headers()`, `draftMode()`, and page/layout `params`/`searchParams` are **async and must be awaited** — the synchronous forms were removed in Next 16, so a sync access is a hard error, not a warning.
  2. Tailwind v4 is **CSS-first**: `globals.css` starts with `@import "tailwindcss";` (never the v3 `@tailwind base/components/utilities` trio). There is **no `tailwind.config.ts`**. Component-style classes (`.card`, `.btn-primary`, …) go in `@layer components`; design tokens that should generate utilities go in `@theme`; plain CSS custom properties stay in `:root {}` (unchanged from v3).
  3. The Supabase session-refresh file is **`proxy.ts`** at the repo root (exporting `async function proxy`), not `middleware.ts` — Next 16 renamed it.
  4. The `@supabase/ssr` cookie handler uses **`getAll`/`setAll`** (never per-cookie `get`/`set`/`remove`, which now error). `setAll` takes a **second `headers` argument** in the proxy that must be applied to the response (`Cache-Control: no-store` hardening — prevents a CDN serving one user's session to another). Server-Component clients wrap `setAll`'s body in `try/catch` (Server Components can't write cookies).
  5. Component tests need **`@testing-library/react` v16+** with **`@testing-library/dom`** installed explicitly as its peer; unit tests run under a jsdom Vitest environment configured in Task 1.
...
proxy.ts                             # Supabase session refresh (Next 16 renamed middleware.ts → proxy.ts)
components/
...
lib/
  supabase/{client.ts,server.ts}      # proxy.ts lives at repo root, not here
...
**Files:**
- Create: `app/layout.tsx`, `app/globals.css`, `vitest.config.ts`, `test/setup.ts`
- Generated by scaffold (do not hand-create): `package.json`, `tsconfig.json`, `next.config.ts`, `postcss.config.mjs`. **No `tailwind.config.ts`** — Tailwind v4 is CSS-first (see Global Constraints).
- Test: `tests/unit/smoke.test.ts`

**Interfaces:**
- Produces: a running `npm run dev` Next.js app with the prototype's theme (CSS vars, fonts, Font Awesome) available globally, and a jsdom Vitest environment that later component tests rely on.
...
Answer prompts to match: TypeScript yes, Tailwind yes, App Router yes, `src/` no. This installs Next 16 + React 19 + Tailwind v4 (CSS-first, no `tailwind.config.ts`) with Turbopack as the default bundler. If the CLI additionally writes an `AGENTS.md`/`CLAUDE.md`, leave it — it doesn't affect the build.

- [ ] **Step 2: Install remaining dependencies**

```bash
npm install @supabase/supabase-js @supabase/ssr chart.js react-chartjs-2 xlsx
npm install -D vitest @vitejs/plugin-react @testing-library/react @testing-library/dom @testing-library/jest-dom @testing-library/user-event jsdom @playwright/test supabase
```

`@testing-library/react` resolves to v16+ (the React 19-compatible line); `@testing-library/dom` is its required peer and must be installed explicitly. No `--legacy-peer-deps` is needed — `react-chartjs-2@5.3.1` already lists `react ^19` in its peer deps.

- [ ] **Step 3: Configure Vitest (jsdom) — required before any component test in later tasks**
...
- [ ] **Step 4: Port the theme into `app/globals.css`**

Replace the generated `app/globals.css` with the Tailwind v4 entry import plus the prototype's CSS variables and classes, ported from `reference/v0.4/finnix-film.html:17-77`. Note the Tailwind v4 structure (see Global Constraints): `@import "tailwindcss";` at the top (not the v3 `@tailwind` trio); the `:root {}` / `html[data-theme]` custom-property blocks are **unchanged** from the prototype; the component-style classes move into `@layer components` so Tailwind utility classes composed alongside them still win the cascade; base element resets, keyframes, and the print block stay unlayered.
...
**Print-isolation approach (differs from the prototype's DOM, so read this carefully).** The prototype hides `#root` and shows `.print-area` because in that single file `.print-area` is a `<body>`-level sibling of `#root`. App Router has no `#root`/`#__next` wrapper, and the ticket's `.print-area` is rendered deep inside the page tree — so a `#root`/`body > *` selector cannot work. Instead: (a) the authenticated shell's outermost `<div>` gets `className="app-shell"` (added in Task 12), and (b) `PrintJobSheet` renders through a React portal to `document.body` (Task 14 Step 3) so it becomes a body-level sibling of `.app-shell`. The print block above then hides `.app-shell` and reveals `.print-area`, reproducing the prototype's on-screen/printed split exactly. This is verified by the print e2e test in Task 21, not here.

- [ ] **Step 5: Wire fonts and Font Awesome in `app/layout.tsx`**
...
- [ ] **Step 6: Smoke test (also proves the jsdom Vitest env from Step 3 loads)**
...
Expected: PASS (2 tests). If `document` is undefined, the `vitest.config.ts` jsdom environment from Step 3 isn't being picked up — fix before proceeding, since every later component test depends on it.

- [ ] **Step 7: Commit**

```bash
git add -A
git commit -m "chore: scaffold Next.js 16 app, Tailwind v4 theme port, jsdom test env"
```
...
### Task 8: Supabase client helpers (browser, server, proxy)

**Files:**
- Create: `lib/supabase/client.ts`, `lib/supabase/server.ts`, `proxy.ts` (repo root — Next 16's renamed middleware)
- Create: `.env.local.example`
- Test: `tests/unit/supabase-client.test.ts`

**Interfaces:**
- Produces: `createClient()` (browser, for Client Components) and `createClient()` (server async, for Server Components/Actions, cookie-based), consumed by every later data-fetching task; plus the root `proxy` session-refresh function.

> Framework note (see Global Constraints): the session-refresh file is `proxy.ts` exporting `async function proxy`, not `middleware.ts`. The `@supabase/ssr` cookie handler uses `getAll`/`setAll`; `setAll` in the proxy takes a second `headers` argument that must be applied to the response.
      ...
      cookies: {
        ...
        setAll: (cookiesToSet) => {
          try {
            ...
            // ignore because the proxy (below) refreshes the session on every request.
          }
        },
      },
    }
...
The `try/catch` is the canonical Server-Component guard: Server Components can't mutate cookies, and without the guard a session refresh triggered during a Server Component render throws. Cookie writes still happen — the proxy does them on the next request.

- [ ] **Step 4: Root `proxy.ts` (session refresh — Next 16's renamed middleware, per `@supabase/ssr`)**

```ts
// proxy.ts  (Next 16 renamed middleware.ts → proxy.ts; exported function is `proxy`)
import { createServerClient } from '@supabase/ssr';
  ...
  const supabase = createServerClient(
    ...
    process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!,
    {
      cookies: {
        ...
        // Second arg `headers` is required in current @supabase/ssr: on auth-cookie
        // writes it carries Cache-Control: no-store, which must reach the response so a
...
```bash
git add lib/supabase proxy.ts .env.local.example tests/unit/supabase-client.test.ts
git commit -m "feat: Supabase browser/server clients and auth proxy"
```
...
export default async function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    // `app-shell` is the print-isolation hook: @media print hides this whole subtree
    // (globals.css, Task 1) so only the body-portaled .print-area shows. Do not remove it.
...
This is the physical work-order sheet technicians print. Port the exact table layout/columns. **Render it through a portal to `document.body`**, not inline in the page tree, so it becomes a body-level sibling of `.app-shell` (Task 12) and the `@media print` rule in globals.css (Task 1) can hide the app and show only this sheet — the App Router equivalent of the prototype's `#root`-sibling `.print-area`:

```tsx
// components/tickets/PrintJobSheet.tsx
'use client';
import { createPortal } from 'react-dom';
import { useEffect, useState } from 'react';

export function PrintJobSheet({ ticket }: { ticket: /* ported ticket shape */ unknown }) {
  const [mounted, setMounted] = useState(false);
  useEffect(() => setMounted(true), []); // portal target (document.body) only exists client-side
  if (!mounted) return null;
  return createPortal(
    <div className="print-area">{/* ported work-order table markup from the prototype */}</div>,
    document.body
  );
}
```

Verify with a manual check (Playwright's `page.emulateMedia({ media: 'print' })` + a visual screenshot comparison, or a manual browser print-preview) rather than a DOM unit test, since the meaningful behavior here is print layout (page size, margins, hidden-on-screen), which `@media print` unit tests can't exercise well — record this as a manual QA step in the task's completion notes, not skipped silently. (The automated `print-job-sheet.spec.ts` in Task 21 asserts the `.app-shell`-hidden / `.print-area`-visible split under print emulation.)
...
(`vitest.config.ts` + `test/setup.ts` already exist from Task 1 — do not recreate them.)
...
- [ ] **Step 3: Remaining e2e specs**, one file each, following the exact login pattern already established in `tests/e2e/tickets.spec.ts` (Task 14 Step 5):
  ...
  - `tests/e2e/print-job-sheet.spec.ts` — navigates to a ticket, calls `page.emulateMedia({ media: 'print' })`, and asserts (via `page.evaluate` reading computed styles) that `.app-shell` computes to `display: none` while `.print-area` computes to `display: block` — i.e. the portal-based print isolation from Tasks 1/12/14 works.
