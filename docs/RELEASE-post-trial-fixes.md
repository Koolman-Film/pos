...
(`release-0019` … `release-0070` และ `repair-categories-and-services.sql`)
...
| 32    | `supabase/release-0051.sql`                   | a normal connection                                 |
...
| 43    | `supabase/release-0062.sql`                   | a normal connection                                 |
...
| 48    | `supabase/release-0067.sql`                   | a normal connection                                 |
| 49    | `supabase/release-0068.sql`                   | a normal connection                                 |
| 50    | `supabase/release-0069.sql`                   | a normal connection                                 |
| 51    | `supabase/release-0070.sql`                   | a normal connection                                 |
| 52    | `supabase/repair-categories-and-services.sql` | a normal connection                                 |
...
| `0049_order_payment_uid_unique`       | uid ของการรับเงินใน PO เดียวกันต้องไม่ซ้ำ ปิดช่องโหว่ที่ payload ส่ง uid เดิมสองครั้งแล้วแถวปลอมได้สถานะ **รับเงินแล้ว** ติดมาด้วย โดยไม่ต้องมีสิทธิ์ `ขายส่ง: ยืนยันเงินเข้า`                                                                                                                                                                                                                                                                                 | ต่ำ. เพิ่ม unique index อย่างเดียว ถ้ามี uid ซ้ำค้างอยู่สคริปต์จะหยุดและบอกว่า PO ไหน โดยไม่แตะข้อมูล                                                                                                                                                             |
| `0050_order_due_date`                 | เพิ่ม `orders.due_at` — กำหนดชำระเงินของ PO พิมพ์ลงในใบแจ้งหนี้และใบส่งของ เว้นว่างได้ถ้ายังไม่ได้ตกลงวันกัน เอกสารก็จะไม่พิมพ์บรรทัดนี้                                                                                                                                                                                                                                                                                                                       | ต่ำ. เพิ่มคอลัมน์ที่เป็น null ได้ PO เดิมไม่มีวันกำหนดชำระ และไม่มีตัวเลขไหนขยับ                                                                                                                                                                                  |
| `0051_adjustment_approval`            | การปรับราคาต้องให้ผู้บริหารอนุมัติ เหมือนการเสนอราคาต่ำกว่ามาตรฐาน **รายการที่ยังไม่อนุมัติจะยังไม่ลดยอดเรียกเก็บ** และขึ้นนับในการ์ดรอการอนุมัติบนแดชบอร์ด ใช้สิทธิ์ `ขายส่ง: อนุมัติ/ปฏิเสธราคา` ตัวเดิม                                                                                                                                                                                                                                                     | ปานกลาง. รายการปรับราคาเดิมถูกเติมเป็น อนุมัติแล้ว ตัวเลขจึงไม่ขยับ แต่ **รายการที่บันทึกหลังจากนี้จะไม่ลดยอดจนกว่าจะมีคนอนุมัติ** ต้องบอกทีมก่อน                                                                                                                 |
...
| `0062_pay_to_account`                 | **บัญชีรับชำระบนใบแจ้งหนี้และใบเสนอราคา** — PO และใบงานเก็บแหล่งเงินที่ให้ลูกค้าโอนเข้า (ต้องเป็นของสาขาเดียวกัน ตรวจด้วย trigger) พิมพ์ลงใบแจ้งหนี้ขายส่งและใบเสนอราคา                                                                                                                                                                                                                                                                                        | ต่ำ. ไม่แก้ข้อมูลเดิม เอกสารที่ไม่ได้เลือกบัญชีพิมพ์เหมือนเดิม ควรใส่ชื่อธนาคารและเลขบัญชีของแหล่งเงินแต่ละอันที่ การจัดการเงิน/บัญชี ก่อนใช้                                                                                                                     |
...
- **No RLS/e2e coverage yet** for `expense_attachments`, `tickets`’ soft-delete columns,
